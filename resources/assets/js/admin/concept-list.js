...
$(function () {
  ...
  var dt_concept_table = $('.datatables-concept');

  // Concepts datatable
  if (dt_concept_table.length) {
    var dt_concept = dt_concept_table.DataTable({
      ajax: dt_concept_table.attr('data-url'),
      processing: true,
      serverSide: true,
      columns: [
        { data: 'id' },
...
(function () {
  ...
  (function () {
    ...
    if (AddConceptForm) {
      fv = FormValidation.formValidation(AddConceptForm, {
        fields: {
          name: {
            validators: {
              notEmpty: {
...
$(document).on('click', '.edit-record', function () {
  ...
  $.get(fetchUrl, function (data) {
    $('#name').val(data.name);
    $('#offcanvasAddConceptLabel').html('Edit Concept');
    $('.submit').html('Update');
    $('.btn.btn-primary.mt-3').val('Update');
    $("#AddConceptForm").attr('action', updateUrl);
    $("#AddConceptForm").append('<input type="hidden" name="_method" value="PUT">')
    if (fv) {
      fv.resetForm();
    }
  });
...
$(document).on('click', '.add-new', function () {
  ...
  $('.submit').html('Submit');
  $("#AddConceptForm").attr('action', storeUrl);
  $("#AddConceptForm").append('<input type="hidden" name="_method" value="POST">');
  resetForm();
});

function resetForm() {
  $('#offcanvasAddUserLabel').html('Add Concept');
  $('#AddConceptForm')[0].reset();
...
$('AddConceptForm').on('hidden.bs.offcanvas', function () {
  fv.resetForm(true);
        ...
        $.ajax({
          ...
          success: function (response) {
            Swal.fire({
              ...
              $('.datatables-concept').DataTable().ajax.reload();
            });
          },
