...
$(function () {
  ...
  var dt_area_table = $('.datatables-area');

  // Areas datatable
  if (dt_area_table.length) {
    var dt_area = dt_area_table.DataTable({
      ajax: dt_area_table.attr('data-url'),
      processing: true,
      serverSide: true,
      columns: [
        { data: 'id' },
...
(function () {
  ...
  (function () {
    ...
    const submitButton = AddAreaForm.querySelector('button[type="submit"]');

    if (AddAreaForm) {
      fv = FormValidation.formValidation(AddAreaForm, {
        fields: {
          name: {
            validators: {
              notEmpty: {
...
$(document).on('click', '.edit-record', function () {
  ...
  $.get(fetchUrl, function (data) {
    ...
    $("#AddAreaForm").attr('action', updateUrl);
    $("#AddAreaForm").append('<input type="hidden" name="_method" value="PUT">');
    if (fv) {
      fv.resetForm();
    }
  });
...
$(document).on('click', '.add-new', function () {
  ...
  $('.submit').html('Submit');
  $("#AddAreaForm").attr('action', storeUrl);
  $("#AddAreaForm").append('<input type="hidden" name="_method" value="POST">');
  resetForm();
});
...
$('#AddAreaForm').on('hidden.bs.offcanvas', function () {
  fv.resetForm(true);
        ...
        $.ajax({
          ...
          success: function (response) {
            Swal.fire({
              ...
              $('.datatables-area').DataTable().ajax.reload();
            });
          },
